...
if constraint_input.lower() == 'yes':
    ...
    with open("constraints.log", "w") as f:
    ...
    for id in tqdm(ids, desc="Molecule count", position=0, ascii=True, leave=True):
        ...
        with open("constraints.log", "a+") as f:
...
if distincting_input.lower() == "yes":
    ...
    from distincting_functions import getRMSD, getNumberOfAtoms
    ...
    print(f"\nFollowing molecules had less than {Nfinal} groups with RMSD > {RMSDBar} (N = {len(fewDistinct)}): {', '.join([f'{i} ({k})' for i, k in fewDistinct.items()])}")
